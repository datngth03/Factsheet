...
{
    public abstract class TbHisPersonBase : Entity<int>
    {
        ...
        protected TbHisPersonBase()
        {

        }

        public TbHisPersonBase(int type, int idPerson, string code, string fullName, DateTime birthDate, bool? isSendMail = null, DateTime? dateSendMail = null, DateTime? insertDate = null, int? companyId = null, string? birthPlace = null, string? address = null, string? iDCardNo = null, DateTime? iDCardDate = null, string? idCardIssuePlace = null, string? ethnicity = null, string? religion = null, string? nationalityCode = null, string? gender = null, string? phone = null, string? email = null, string? note = null, string? image = null, bool? isActive = null, byte? docStatus = null, bool? isCheckRetirement = null, bool? isCheckTermEnd = null, DateTime? crt_date = null, int? crt_user = null, DateTime? mod_date = null, int? mod_user = null, string? oldCode = null)
        {

            Check.NotNull(code, nameof(code));
            Check.Length(code, nameof(code), TbHisPersonConsts.CodeMaxLength, 0);
            Check.NotNull(fullName, nameof(fullName));
            Check.Length(fullName, nameof(fullName), TbHisPersonConsts.FullNameMaxLength, 0);
            Check.Length(birthPlace, nameof(birthPlace), TbHisPersonConsts.BirthPlaceMaxLength, 0);
            Check.Length(address, nameof(address), TbHisPersonConsts.AddressMaxLength, 0);
            Check.Length(iDCardNo, nameof(iDCardNo), TbHisPersonConsts.IDCardNoMaxLength, 0);
            Check.Length(idCardIssuePlace, nameof(idCardIssuePlace), TbHisPersonConsts.IdCardIssuePlaceMaxLength, 0);
            Check.Length(ethnicity, nameof(ethnicity), TbHisPersonConsts.EthnicityMaxLength, 0);
            Check.Length(religion, nameof(religion), TbHisPersonConsts.ReligionMaxLength, 0);
            Check.Length(nationalityCode, nameof(nationalityCode), TbHisPersonConsts.NationalityCodeMaxLength, 0);
            Check.Length(gender, nameof(gender), TbHisPersonConsts.GenderMaxLength, 0);
            Check.Length(phone, nameof(phone), TbHisPersonConsts.PhoneMaxLength, 0);
            Check.Length(email, nameof(email), TbHisPersonConsts.EmailMaxLength, 0);
            Check.Length(note, nameof(note), TbHisPersonConsts.NoteMaxLength, 0);
            Check.Length(oldCode, nameof(oldCode), TbHisPersonConsts.OldCodeMaxLength, 0);
            Type = type;
            IdPerson = idPerson;
            Code = code;
            FullName = fullName;
