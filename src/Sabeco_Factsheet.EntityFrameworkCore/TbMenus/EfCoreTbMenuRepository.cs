...
{
    public abstract class EfCoreTbMenuRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbMenu, int>
    {
        public EfCoreTbMenuRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, controlName, description, isActive, create_userMin, create_userMax, create_dateMin, create_dateMax, mod_userMin, mod_userMax, mod_dateMin, mod_dateMax);

            var ids = query.Select(x => x.Id);
            await DeleteManyAsync(ids, cancellationToken: GetCancellationToken(cancellationToken));
        }

        public virtual async Task<List<TbMenu>> GetListAsync(
            string? filterText = null,
            string? controlName = null,
            string? description = null,
            bool? isActive = null,
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, controlName, description, isActive, create_userMin, create_userMax, create_dateMin, create_dateMax, mod_userMin, mod_userMax, mod_dateMin, mod_dateMax);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbMenu> ApplyFilter(
            IQueryable<TbMenu> query,
            string? filterText = null,
            string? controlName = null,
            string? description = null,
            bool? isActive = null,
        ...
        {
            return query
                    .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.ControlName.ToLower().Contains(filterText.ToLower()) || e.Description.ToLower().Contains(filterText.ToLower()))
                    .WhereIf(!string.IsNullOrWhiteSpace(controlName), e => e.ControlName.ToLower().Contains(controlName.ToLower()))
