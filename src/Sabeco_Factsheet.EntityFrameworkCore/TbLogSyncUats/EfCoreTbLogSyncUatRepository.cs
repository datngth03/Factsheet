...
{
    public abstract class EfCoreTbLogSyncUatRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbLogSyncUat, int>
    {
        public EfCoreTbLogSyncUatRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, timeExportMin, timeExportMax, isSuccess, userExportMin, userExportMax, reasonExportFailed);

            var ids = query.Select(x => x.Id);
            await DeleteManyAsync(ids, cancellationToken: GetCancellationToken(cancellationToken));
        }

        public virtual async Task<List<TbLogSyncUat>> GetListAsync(
            string? filterText = null,
            DateTime? timeExportMin = null,
            DateTime? timeExportMax = null,
            bool? isSuccess = null,
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, timeExportMin, timeExportMax, isSuccess, userExportMin, userExportMax, reasonExportFailed);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbLogSyncUat> ApplyFilter(
            IQueryable<TbLogSyncUat> query,
            string? filterText = null,
            DateTime? timeExportMin = null,
            DateTime? timeExportMax = null,
            bool? isSuccess = null,
        ...
        {
            return query
                    .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.ReasonExportFailed.ToLower().Contains(filterText.ToLower()))
