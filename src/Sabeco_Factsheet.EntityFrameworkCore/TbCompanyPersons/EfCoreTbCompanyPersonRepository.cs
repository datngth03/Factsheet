...
{
    public abstract class EfCoreTbCompanyPersonRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbCompanyPerson, int>
    {
        public EfCoreTbCompanyPersonRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, branchCode, companyIdMin, companyIdMax, personIdMin, personIdMax, positionIdMin, positionIdMax, fromDateMin, fromDateMax, toDateMin, toDateMax, positionCode, postionTypeMin, postionTypeMax, personalValueMin, personalValueMax, personalSharePercentageMin, personalSharePercentageMax, ownerValueMin, ownerValueMax, representativePercentageMin, representativePercentageMax, note, isActive, crt_dateMin, crt_dateMax, crt_userMin, crt_userMax, mod_dateMin, mod_dateMax, mod_userMin, mod_userMax, isDeleted);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbCompanyPerson> ApplyFilter(
            IQueryable<TbCompanyPerson> query,
            string? filterText = null,
            string? branchCode = null,
            int? companyIdMin = null,
            int? companyIdMax = null,
