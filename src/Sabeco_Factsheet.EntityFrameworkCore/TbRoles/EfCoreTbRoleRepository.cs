...
{
    public abstract class EfCoreTbRoleRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbRole, int>
    {
        public EfCoreTbRoleRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, code, name, description, isActive, crt_dateMin, crt_dateMax, crt_userMin, crt_userMax, mod_dateMin, mod_dateMax, mod_userMin, mod_userMax);

            var ids = query.Select(x => x.Id);
            await DeleteManyAsync(ids, cancellationToken: GetCancellationToken(cancellationToken));
        }

        public virtual async Task<List<TbRole>> GetListAsync(
            string? filterText = null,
            string? code = null,
            string? name = null,
            string? description = null,
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, code, name, description, isActive, crt_dateMin, crt_dateMax, crt_userMin, crt_userMax, mod_dateMin, mod_dateMax, mod_userMin, mod_userMax);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbRole> ApplyFilter(
            IQueryable<TbRole> query,
            string? filterText = null,
            string? code = null,
            string? name = null,
            string? description = null,
        ...
        {
            return query
                    .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.Code.ToLower().Contains(filterText.ToLower()) || e.Name.ToLower().Contains(filterText.ToLower()) || e.Description.ToLower().Contains(filterText.ToLower()))
