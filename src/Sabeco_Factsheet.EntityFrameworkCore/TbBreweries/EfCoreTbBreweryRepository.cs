...
{
    public abstract class EfCoreTbBreweryRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbBrewery, int>
    {
        public EfCoreTbBreweryRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, breweryCode, breweryName, breweryName_EN, briefName, breweryAdress, companyIdMin, companyIdMax, capacityVolumeMin, capacityVolumeMax, deliveryVolumeMin, deliveryVolumeMax, note, docStatusMin, docStatusMax, isActive, create_userMin, create_userMax, create_dateMin, create_dateMax, mod_userMin, mod_userMax, mod_dateMin, mod_dateMax);

            var ids = query.Select(x => x.Id);
            await DeleteManyAsync(ids, cancellationToken: GetCancellationToken(cancellationToken));
        }

        public virtual async Task<List<TbBrewery>> GetListAsync(
            string? filterText = null,
            string? breweryCode = null,
            string? breweryName = null,
            string? breweryName_EN = null,
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, breweryCode, breweryName, breweryName_EN, briefName, breweryAdress, companyIdMin, companyIdMax, capacityVolumeMin, capacityVolumeMax, deliveryVolumeMin, deliveryVolumeMax, note, docStatusMin, docStatusMax, isActive, create_userMin, create_userMax, create_dateMin, create_dateMax, mod_userMin, mod_userMax, mod_dateMin, mod_dateMax);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbBrewery> ApplyFilter(
            IQueryable<TbBrewery> query,
            string? filterText = null,
            string? breweryCode = null,
            string? breweryName = null,
            string? breweryName_EN = null,
