...
{
    public abstract class EfCoreTbBrewerySkuRepositoryBase : EfCoreRepository<Sabeco_FactsheetDbContext, TbBrewerySku, int>
    {
        public EfCoreTbBrewerySkuRepositoryBase(IDbContextProvider<Sabeco_FactsheetDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, yearMin, yearMax, breweryCode, sKUCode, productionVolumeMin, productionVolumeMax, docStatusMin, docStatusMax, isActive, crt_dateMin, crt_dateMax, crt_userMin, crt_userMax, mod_dateMin, mod_dateMax, mod_userMin, mod_userMax, breweryIdMin, breweryIdMax, sKUIdMin, sKUIdMax);

            var ids = query.Select(x => x.Id);
            await DeleteManyAsync(ids, cancellationToken: GetCancellationToken(cancellationToken));
        }

        public virtual async Task<List<TbBrewerySku>> GetListAsync(
            string? filterText = null,
            int? yearMin = null,
            int? yearMax = null,
            string? breweryCode = null,
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, yearMin, yearMax, breweryCode, sKUCode, productionVolumeMin, productionVolumeMax, docStatusMin, docStatusMax, isActive, crt_dateMin, crt_dateMax, crt_userMin, crt_userMax, mod_dateMin, mod_dateMax, mod_userMin, mod_userMax, breweryIdMin, breweryIdMax, sKUIdMin, sKUIdMax);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<TbBrewerySku> ApplyFilter(
            IQueryable<TbBrewerySku> query,
            string? filterText = null,
            int? yearMin = null,
            int? yearMax = null,
            string? breweryCode = null,
